 ...
 * Developer: Wilfredo Palma
 * Developer URI: https://wilfre90dev.github.com
 * Text Domain: woocommerce-cuba-places
 ...
 */
defined( 'ABSPATH' ) or die( __('You can not access this file directly!', 'woocommerce-cuba-places') );
...
if(in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    ...
    //	Declare plugin’s compatibility with HPOS
    ...
    class WC_Cuban_Places {
        ...
        public function init_textdomain() {
            load_plugin_textdomain('woocommerce-cuba-places', FALSE,	dirname(plugin_basename(__FILE__)) . '/languages' );
        }
        ...
        public function  wc_states() {
            ...
            if (!empty( $allowed ) ) {
                foreach ($allowed as $code => $country) {
                    if (! isset( $states[$code] ) && file_exists($this->get_plugin_path() . '/includes/provinces.php')) {
                        include($this->get_plugin_path() . '/includes/provinces.php');
                    }
                }
            }
        ...
        public function load_country_places() {
            ...
            if ( $allowed ) {
                foreach ( $allowed as $code => $country ) {
                    if ( ! isset( $places[ $code ] ) && file_exists( $this->get_plugin_path() . '/includes/municipalities.php' ) ) {
                        include( $this->get_plugin_path() . '/includes/municipalities.php' );
                    }
                }
            }
     ...
     * Instantiate class
     */
    $GLOBALS['wc_cuban_places'] = new WC_Cuban_Places();
};
